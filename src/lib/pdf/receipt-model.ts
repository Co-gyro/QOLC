...
function buildInsuranceFullDetail(
  ...
  const note =
    mode === "iryou"
      ? "費用総額・保険給付額・自己負担額は、請求ソフトで確定済みの金額を各項目の費用・回数に基づいて按分表示したものです（各列の合計は確定額と一致）。利用者負担額は10円未満四捨五入。"
      : "費用総額・保険給付額・自己負担額は、請求ソフトで確定済みの金額を各サービスの単位数・回数に基づいて按分表示したものです（各列の合計は確定額と一致）。";
