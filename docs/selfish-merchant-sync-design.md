...
#### Selfish 側の対応（2026-09-16 完了・commit 9f24778）

`PAYLOAD_SCHEMA` を `qolc.merchant.v2` に上げ、v1 は「版が違う」として拒否する。

必須の範囲は **`card_numbers` に載せたブランドの分だけ**。

- 足りないキーは**そのブランドを名指しで**拒否する。既定値では埋めない
  （この料率は店子への振込額に出ず UD/NM 按分だけが静かにずれるため、
  埋めると誤りに誰も気づけない。0014 と同じ理由）
- 載せていないブランドの余分なキーは**無視する**。ここで落とすと、
  QOLC 側が「常に両ブランドを送る」実装に変えたときに全件通らなくなる
- v1 の単一欄が来た場合も「ブランド別の形で入っていない」と返す
  （版チェックで先に落ちるが、版だけ上げて中身が古い送信への保険）
- 形式の検証もブランドごとに行い、壊れている側を名指しする。
  ブランド共通の項目（開始日・加盟店手数料率）は1回だけ報告する

実装上の注意: 加盟店番号の解析を料率より**前**に置く。
どのブランドの率が必須かは `card_numbers` が決めるため。

テスト: 単体 866件 / E2E 147件緑（HTTP 経由で実ルートを通すE2Eを含む）。

**未実施**: 本番へ v2 のペイロードを実際に署名して送る確認。
共有鍵は Vercel の Secret 型で読み出せず（`vercel env pull` は `[SENSITIVE]` を返す）、
生成した鍵も手元に残していないため署名できない。
v2 の変更は純粋な検証ロジックで実行環境に依存しないうえ、
HTTP 経由の E2E で同じルートを通しているが、**本番での最終確認は
QOLC の「Selfish へ登録」を初めて押したときになる**。
