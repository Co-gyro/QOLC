...
VALUES (
  ...
  },
  {
    ...
    "guide": "QOLC の加盟店管理で該当加盟店の「Selfish登録」を開き、チェックリストの不足（銀行コード・支店コード・全銀カナ名義・料率適用開始日は申請詳細のUD追記情報で入力）を解消してから「Selfish へ登録」を押す。連携未設定時は登録票を見ながら Selfish で 法人→店舗→振込口座→加盟店番号→料率 の順に手動登録する。Selfish の明細突合キーは店子の加盟店番号（JCB 14桁・セゾンは加盟店No.7桁+店舗No.7桁）で、CSVファイル名の支払先番号ではない。完了条件: 登録結果（created/updated）が案件のタイムラインに記録されていること。",
    "external_url": "/admin/merchants",
    "external_label": "加盟店管理を開く"
  },
  {
