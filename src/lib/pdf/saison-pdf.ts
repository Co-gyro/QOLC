...
export function parseSaisonPdf(text: string): SaisonPdfData {
...
export async function parseSaisonPdfFromFile(file: File): Promise<SaisonPdfData> {
  const buffer = await file.arrayBuffer();
  const text = await extractPdfText(buffer);
  return parseSaisonPdf(text);
}
