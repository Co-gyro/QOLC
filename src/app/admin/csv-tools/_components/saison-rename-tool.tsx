...
import {
  detectSaisonUrFromFile,
  type SaisonUrDetectionResult,
...
interface FileEntry {
  id: string;
  file: File;
  detection: SaisonUrDetectionResult | null;
  error: string | null;
  loading: boolean;
}
...
export function SaisonRenameTool() {
  ...
  const addFiles = useCallback((files: File[]) => {
    ...
    newEntries.forEach((entry) => {
      detectSaisonUrFromFile(entry.file)
        .then((result) => {
          setEntries((prev) =>
            prev.map((e) =>
              e.id === entry.id ? { ...e, detection: result, loading: false } : e,
            ),
          );
        })
        .catch((err: unknown) => {
  ...
  const canDownload = useMemo(
    () =>
      ...
      entries.some((e) => e.detection?.isSaisonUr),
    [closingDate, payeeNumber, entries],
  );

  const handleDownload = useCallback(
    async (entry: FileEntry) => {
      if (!entry.detection?.isSaisonUr) return;
  ...
  const handleDownloadAll = useCallback(async () => {
    for (const entry of entries) {
      if (!entry.detection?.isSaisonUr) continue;
  ...
  const handleDownloadZip = useCallback(async () => {
    if (!isValidClosingDate(closingDate) || !isValidPayeeNumber(payeeNumber)) return;
    const targets = entries.filter((e) => e.detection?.isSaisonUr);
    ...
    try {
      ...
      const zipName = `SAISON_UR_${formatClosingDate(closingDate)}.zip`;
  ...
  return (
    <div className="grid gap-6">
      <Card>
        <CardHeader>
          <CardTitle>セゾン 売上明細 (UR) リネーム</CardTitle>
          <CardDescription>
            NetアンサーforBizからダウンロードした売上データCSVを、
            セルフィッシュ命名規則 (SAISON_UR_締日_支払先番号.csv) にリネームします。
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="saison-ur-closing-date">締日</Label>
              <Input
                id="saison-ur-closing-date"
                type="date"
                value={closingDate}
            ...
            <div className="space-y-2">
              <Label htmlFor="saison-ur-payee-number">支払先番号（9桁）</Label>
              <Input
                id="saison-ur-payee-number"
                type="text"
                inputMode="numeric"
                maxLength={9}
        ...
        <Card>
          <CardHeader className="flex-row items-center justify-between space-y-0">
            <div>
              ...
              <CardDescription>
                セゾン売上明細CSVであることをヘッダー列から自動判定しています。
              </CardDescription>
            </div>
          ...
          <CardContent>
            <ul className="divide-y">
              {entries.map((entry) => {
                const ok = entry.detection?.isSaisonUr ?? false;
                const newName =
                  ok && isValidClosingDate(closingDate) && isValidPayeeNumber(payeeNumber)
                    ? buildCsvFilename({
                        issuer: "SAISON",
                        dataType: "UR",
                        closingDate,
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <span
                          className={cn(
                            ...
                            ok ? "bg-blue-100 text-blue-700" : "bg-muted text-muted-foreground",
                          )}
                        >
                          {entry.loading ? "判定中…" : ok ? "売上明細 (UR)" : "判定不可"}
                        </span>
